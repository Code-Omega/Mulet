...
Network = () ->
  ...
  network = (selection, data) ->
    ...
    namesG = vis.append("g").attr("id", "names")
  ...
  # Public function to switch between labeling options
  network.toggleName = (newName) ->
    force.stop()
    setName(newName)
    update()
  ...
  setupData = (data) ->
    ...
    countExtent = d3.extent(data.nodes, (d) -> d.activation)
    circleRadius = d3.scale.sqrt().range([6, 36]).domain(countExtent)
    ...
    data.nodes.forEach (n) ->
      ...
      n.radius = circleRadius(n.activation)
  ...
  filterNodes = (allNodes) ->
    ...
    if filter == "popular" or filter == "obscure"
      activations = allNodes.map((d) -> d.activation).sort(d3.ascending)
      cutoff = d3.quantile(activations, 0.5)
  ...
  updateCenters = (types) ->
    if layout == "radial"
      groupCenters = RadialPlacement().center({"x":width/2, "y":height / 2 - 50})
        .radius(150).increment(9).keys(types)
  ...
  updateNodes = () ->
    ...
    name = namesG.selectAll("text.name")
      .data(curNodesData, (d) -> d.id)
...
$ ->
  ...
  d3.selectAll("#names a").on "click", (d) ->
    newName = d3.select(this).attr("id")
    activate("names", newName)
    myNetwork.toggleName(newName)

  $("#section_select").on "change", (e) ->
    songFile = $(this).val()
